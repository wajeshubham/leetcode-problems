...
var snakesAndLadders = function (board) {
  ...
  function intToPos(square) {
    ...
    if (row % 2 == 1) {
      ...
      //12 <- 11 <- 10 <- 9 <- 8 <- 7 (column for 7 is 5 (0 indexed) but if we calculate based on (square - 1) % length it calculates to 0 so we subtract it from the (board length - 1) to flip)
      //                            |(up)
      // 1 -> 2 -> 3 -> 4 -> 5 ->  6
